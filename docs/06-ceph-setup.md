...
Create the initial monitor on n1:

**GUI Method:**
1. Navigate: n1 → Ceph → Monitor
2. Click: "Create"
...
ssh n1 "ceph config set global public_network 10.100.0.0/24"
ssh n1 "ceph config set global cluster_network 10.100.0.0/24"

# Option B: Split networks (if VMs need external Ceph access)
# Public network (client I/O) - use your fast ethernet
ssh n1 "ceph config set global public_network 10.11.11.0/24"

# Cluster network (OSD replication) - THIS IS YOUR TB4 NETWORK!
ssh n1 "ceph config set global cluster_network 10.100.0.0/24"

# Also set for monitors explicitly
ssh n1 "ceph config set mon public_network 10.11.11.0/24"
ssh n1 "ceph config set mon cluster_network 10.100.0.0/24"
```

**Verify settings:**
```bash
ssh n1 "ceph config get mon public_network"
ssh n1 "ceph config get mon cluster_network"
```

### Step 5: Create Additional Monitors

Now create monitors on n2 and n3:

**GUI Method:**
1. n2 → Ceph → Monitor → Create
2. n3 → Ceph → Monitor → Create

**CLI Method:**
```bash
ssh n2 "pveceph mon create --mon-address 10.100.0.13"
ssh n3 "pveceph mon create --mon-address 10.100.0.14"
```

**Verify 3-node quorum:**
```bash
ssh n1 "ceph quorum_status --format json-pretty | head -20"
```
...
```bash
# On n1
ssh n1 "pveceph osd create /dev/nvme1n1"
ssh n1 "pveceph osd create /dev/nvme2n1"
...
ssh n1 "ceph osd pool create cephtb4 256 256"

# Set 2:1 replication (size=2, min_size=1)
ssh n1 "ceph osd pool set cephtb4 size 2"
ssh n1 "ceph osd pool set cephtb4 min_size 1"

# Enable RBD application
ssh n1 "ceph osd pool application enable cephtb4 rbd"
```
...
ssh n1 "ceph config get osd cluster_network"
# Should show: 10.100.0.0/24

# Watch OSD traffic on TB4 interface
ssh n1 "iftop -i en05"  # Ctrl+C to exit
```
...
```bash
# Check OSD status
ssh n1 "systemctl status ceph-osd@0"

# Check logs
ssh n1 "journalctl -u ceph-osd@0 -n 50"
```
...
ssh n1 "ip addr show en05 en06"

# Bring up interfaces
ssh n1 "/usr/local/bin/pve-en05.sh"
ssh n1 "/usr/local/bin/pve-en06.sh"

# Restart OSDs
ssh n1 "systemctl restart ceph-osd.target"
```
